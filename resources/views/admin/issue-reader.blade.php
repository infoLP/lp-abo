...
<head>
    ...
    <title>Prévisualisation — {{ $issue->magazine->name }} N°{{ $issue->issue_number }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #toolbar .badge-admin {
            background: #744210; color: #fbd38d;
        ...
        #loader-bar-bg {
            width: 300px; height: 6px; background: #2d3748; border-radius: 3px; overflow: hidden;
        }
        #loader-bar { height: 100%; background: #744210; width: 0%; transition: width .3s; border-radius: 3px; }
...
<!-- Barre d'outils -->
<div id="toolbar">
    <button onclick="window.close()" title="Fermer">✕ Fermer</button>
...
const PDF_URL   = "{{ route('admin.preview.stream', $issue) }}";
    ...
    return pdfDoc.getPage(num).then(page => {
        ...
        return page.render({ canvasContext: canvas.getContext('2d'), viewport: vp }).promise
            ...
            .then(tc => {
                ...
                const done = container.querySelectorAll('.page-wrapper').length;
                loaderBar.style.width = Math.round((done / pageCount) * 100) + '%';
            });
    });
}
...
async function doSearch(query) {
    ...
    document.querySelectorAll('.page-wrapper').forEach(wrapper => {
        const tc = wrapper._textContent, vp = wrapper._viewport, num = wrapper._pageNum;
        if (!tc) return;
